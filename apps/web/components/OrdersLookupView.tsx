...
interface OrdersLookupViewProps {
  embedded?: boolean;
  onBrowseProducts?: () => void;
}

export default function OrdersLookupView({ embedded = false, onBrowseProducts }: OrdersLookupViewProps) {
  ...
  useEffect(() => {
    fetchAppSettings().then((s) => {
      if (s?.contact?.whatsapp) setWhatsapp(s.contact.whatsapp.replace(/[^0-9]/g, ""));
    });
  }, []);
  ...
  return (
    <div style={{ width: "100%", maxWidth: 760 }}>
      ...
      {/* Search Bar */}
      <form onSubmit={handleSearch} style={{ maxWidth: 500, marginBottom: 28 }}>
      ...
      {loading ? (
        ...
        <div className="empty-state" style={{ maxWidth: 500, margin: "0 auto", padding: "40px 20px" }}>
          ...
          {onBrowseProducts ? (
            <button type="button" onClick={onBrowseProducts} className="btn btn--primary">
              Start Shopping
            </button>
          ) : (

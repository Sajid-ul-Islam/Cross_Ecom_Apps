...
/**
 * Fetches the social media content feed and tagged products from Fastify gateway.
 * Falls back to high-fidelity brand curated feed on network failure.
 */
export async function fetchSocialFeed(): Promise<SocialFeedResponse> {
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 4000);
    const res = await fetch(`${GATEWAY_URL}/v1/deen/social/feed`, {
